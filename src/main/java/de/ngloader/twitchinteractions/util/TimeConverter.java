...
public enum TimeConverter {
	...
	public static TimeConverter suggestFirst(String input) {
	...
	public static List<TimeConverter> suggest(String input) {
		input = input.toLowerCase();

		List<TimeConverter> result = null;
		for (TimeConverter time : values()) {
			if (time.name().toLowerCase().startsWith(input)) {
				if (result == null) {
	...
	private TimeConverter(long part, long inTicks) {
		this.part = part;
		this.ticks = inTicks;
	}
