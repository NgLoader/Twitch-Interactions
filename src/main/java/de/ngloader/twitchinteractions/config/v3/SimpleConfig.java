...
public class SimpleConfig<Config> {
	...
	private <Type, Require extends Annotation> void serialize(ConfigurationSection config, Object instance) throws Exception {
		for (SimpleField<?> field : SimpleField.getFields(instance.getClass())) {
			...
			if (translatorKey == null) {
				TILogger.info("No SimpleKey annotation found in section " + config.getName() + " (Class: " + this.configClass.getSimpleName() + ")");
				continue;
			}
			...
			if (field.isSection()) {
				...
				if (newSectionInstance == null && field.isChildSection()) {
					try {
						...
						TILogger.error("Unable to create config section for " + newSectionName + " in " + this.configClass.getSimpleName(), e);
					}
				}
	...
	private <Type, Require extends Annotation> Type deserialize(ConfigurationSection config, Class<? extends Type> configClass) throws Exception {
		...
		for (SimpleField<?> field : SimpleField.getFields(configClass)) {
			...
			if (value != null) {
				if (requireAnnotation != null) {
					...
					if (result != null && !result.equals(value)) {
						TILogger.warn("Unsing default value (" + result + ") for key \"" + translatorKey.name() + "\" in section " + config.getName());
					}
			...
			TILogger.warn("Unsing default value (" + defaultValue + ") for key \"" + translatorKey.name() + "\" in section " + config.getName());
	...
	public Config getOrDeserializeConfig() {
		if (this.instance == null) {
			try {
				...
				TILogger.error("Unable to deserialize config class of " + this.configClass.getSimpleName(), e);
			}
		}
		return this.instance;
	}
